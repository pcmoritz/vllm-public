...
void fp8_scaled_gemm(torch::Tensor& out, torch::Tensor& input, torch::Tensor& weights, torch::Tensor& scales, torch::Tensor& workspace) {
  ...
  TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutCreate(&Adesc, CUDA_R_8F_E4M3, transa == CUBLAS_OP_N ? m : k, transa == CUBLAS_OP_N ? k : m, lda));
  TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutCreate(&Bdesc, CUDA_R_8F_E4M3, transb == CUBLAS_OP_N ? k : n, transb == CUBLAS_OP_N ? n : k, ldb));
  TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutCreate(&Cdesc, CUDA_R_16BF, m, n, ldc));
  TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutCreate(&Ddesc, CUDA_R_8F_E4M3, m, n, ldc));
  ...
  TORCH_CUDABLAS_CHECK(cublasLtMatmulAlgoGetHeuristic(ltHandle, operationDesc, Adesc, Bdesc, Cdesc, Ddesc, preference, 1, &heuristicResult, &returnedResults));

  if (returnedResults == 0) {
      TORCH_CUDABLAS_CHECK(CUBLAS_STATUS_NOT_SUPPORTED);
  }

  TORCH_CUDABLAS_CHECK(cublasLtMatmul(ltHandle,
                                   operationDesc,
                                   &alpha,
                                   A,
                                   Adesc,
  ...
  if (preference) TORCH_CUDABLAS_CHECK(cublasLtMatmulPreferenceDestroy(preference));
  if (Ddesc) TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutDestroy(Ddesc));
  if (Cdesc) TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutDestroy(Cdesc));
  if (Bdesc) TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutDestroy(Bdesc));
  if (Adesc) TORCH_CUDABLAS_CHECK(cublasLtMatrixLayoutDestroy(Adesc));
  if (operationDesc) TORCH_CUDABLAS_CHECK(cublasLtMatmulDescDestroy(operationDesc));
}
