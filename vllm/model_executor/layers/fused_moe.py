...
def fused_moe_kernel1(
    ...
    offs_cn = pid_n * (BLOCK_SIZE_N // 2) + tl.arange(0, BLOCK_SIZE_N)
    c_ptrs = c_ptr + stride_cm * offs_token[:, None] + stride_cn * offs_cn[
        None, :]
    c_mask = token_mask[:, None] & (offs_cn[None, :] < (pid_n + 1) * (BLOCK_SIZE_N // 2))
...
def invoke_fused_moe_kernel1(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor,
...
def invoke_fused_moe_kernel2(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor,
                            topk_weights: torch.Tensor, topk_ids: torch.Tensor,
                            sorted_token_ids: torch.Tensor,
                            expert_ids: torch.Tensor,
                            num_tokens_post_padded: torch.Tensor,
...
def fused_moe(hidden_states: torch.Tensor,
    ...
    invoke_fused_moe_kernel1(hidden_states, w1, intermediate_cache2,
                            topk_weights, topk_ids, sorted_token_ids,
                            expert_ids, num_tokens_post_padded, False,
                            topk_ids.shape[1], config)

    invoke_fused_moe_kernel2(intermediate_cache2.view(M * topk_ids.shape[1], N // 2),
