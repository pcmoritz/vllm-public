...
def fused_moe_kernel(
    ...
    if not FUSE_SILU:
        ...
        idx1 = tl.arange(0, BLOCK_SIZE_N//2)
        idx2 = tl.arange(BLOCK_SIZE_N//2, BLOCK_SIZE_N)
        x = accumulator[:,idx1]
        y = accumulator[:,idx2]
        result = x * tl.sigmoid(y).to(compute_type)
...
def invoke_fused_moe_kernel(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor,
                            ...
                            mul_routed_weight: bool, fuse_silu: bool, top_k: int, config: dict):
    ...
    fused_moe_kernel[grid](
        ...
        B.stride(1),
        C.stride(1),
        C.stride(2) * (2 if fuse_silu else 1),
